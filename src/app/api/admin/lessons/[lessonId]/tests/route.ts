import { NextResponse } from 'next/server'
import { z } from 'zod'
import { getAdminUser } from '@/lib/admin-access'
import { db } from '@/lib/db'
import { AcademyAssessmentError, upsertLessonTest } from '@/server/services/academy-assessment-service'

const QuestionSchema = z.object({
  title: z.string().trim().min(1),
  description: z.string().trim().optional().nullable(),
  options: z.array(z.string().trim().min(1)).min(2),
  correctAnswer: z.string().trim().min(1),
}).refine((question) => question.options.includes(question.correctAnswer), {
  message: 'La respuesta correcta debe ser una de las opciones.',
  path: ['correctAnswer'],
})

const LessonTestSchema = z.object({
  title: z.string().trim().min(1),
  description: z.string().trim().optional().nullable(),
  maxAttempts: z.number().int().min(1).max(50).default(1),
  passingScore: z.number().int().min(0).max(100).default(70),
  questions: z.array(QuestionSchema).optional(),
})

export async function GET(_: Request, { params }: { params: Promise<{ lessonId: string }> }) {
  const admin = await getAdminUser()
  if (!admin) return NextResponse.json({ success: false, error: 'Acceso de administración requerido.' }, { status: 403 })
  const { lessonId } = await params
  const lesson = await db.lesson.findUnique({ where: { id: lessonId }, select: { id: true } })
  if (!lesson) return NextResponse.json({ success: false, error: 'La lección no existe.' }, { status: 404 })

  const tests = await db.lessonTest.findMany({
    where: { lessonId },
    include: { questions: { orderBy: { order: 'asc' } }, _count: { select: { submissions: true } } },
    orderBy: { order: 'asc' },
  })
  return NextResponse.json({ success: true, data: tests.map(({ _count, ...test }) => ({ ...test, submissionCount: _count.submissions })) })
}

export async function POST(request: Request, { params }: { params: Promise<{ lessonId: string }> }) {
  try {
    const admin = await getAdminUser()
    if (!admin) return NextResponse.json({ success: false, error: 'Acceso de administración requerido.' }, { status: 403 })
    const { lessonId } = await params
    const input = LessonTestSchema.parse(await request.json())
    const test = await upsertLessonTest(lessonId, input)
    return NextResponse.json({ success: true, data: test }, { status: 201 })
  } catch (error) {
    if (error instanceof z.ZodError) return NextResponse.json({ success: false, error: 'La configuración del test no es válida.', details: error.issues }, { status: 400 })
    if (error instanceof AcademyAssessmentError) return NextResponse.json({ success: false, error: error.message, code: error.code }, { status: error.status })
    console.error('Error creating lesson test:', error)
    return NextResponse.json({ success: false, error: 'No fue posible crear el test.' }, { status: 500 })
  }
}
