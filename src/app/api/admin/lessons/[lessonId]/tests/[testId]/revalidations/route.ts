...
import { AcademyAssessmentError, grantLessonTestRevalidation } from '@/server/services/academy-assessment-service'
...
export async function POST(
  request: Request,
  { params }: { params: Promise<{ lessonId: string; testId: string }> }
) {
  try {
    ...
    const data = await grantLessonTestRevalidation(admin.id, lessonId, testId, input.userId, input.attemptsGranted, input.reason)
    ...
    console.error('Error granting lesson test revalidation:', error)
