...
type Detail = { assessment: { id: string; title: string; description?: string | null; isFinalExam: boolean; passingScore?: number; questions: Question[] }; remainingAttempts: number; canSubmit: boolean; waitingForReview: boolean; exhausted: boolean; unavailableReason: string | null; attempts: Array<{ id: string; status: string; score?: number | null; reviewNote?: string | null }> }
type Answer = { responseText?: string; fileUrl?: string; fileMimeType?: string }

/**
 * Turns the raw attempt status into something a student can act on. Showing the
 * enum ("APPROVED") tells them nothing about what to do next.
 */
function AttemptResult({ attempt, passingScore }: { attempt: { status: string; score?: number | null; reviewNote?: string | null }; passingScore?: number }) {
  const tone = attempt.status === 'APPROVED'
    ? { box: 'border-emerald-500/30 bg-emerald-500/10', text: 'text-emerald-300', icon: '✓', headline: 'Aprobaste esta evaluación' }
    : attempt.status === 'NOT_PASSED'
      ? { box: 'border-red-500/30 bg-red-500/10', text: 'text-red-300', icon: '✕', headline: 'No alcanzaste la nota mínima' }
      : { box: 'border-ap-copper/30 bg-ap-copper/10', text: 'text-ap-copper', icon: '⏳', headline: 'Pendiente de corrección' }
  ...
  return (
    <div className={`rounded-xl border p-4 ${tone.box}`}>
      <div className="flex flex-wrap items-center gap-2">
        <span aria-hidden="true" className={`text-lg ${tone.text}`}>{tone.icon}</span>
        <p className={`font-semibold ${tone.text}`}>{tone.headline}</p>
        {hasScore && (
          <span className={`ml-auto rounded-full bg-black/25 px-3 py-1 text-sm font-bold ${tone.text}`}>
            {Math.round(attempt.score as number)}%
          </span>
        )}
      </div>
      {hasScore && typeof passingScore === 'number' && (
        <p className="mt-2 text-xs text-zinc-400">Nota mínima requerida: {passingScore}%</p>
      )}
      {attempt.reviewNote && (
        <p className="mt-3 border-t border-white/10 pt-3 text-sm text-zinc-300">
          <span className="text-zinc-500">Comentario: </span>{attempt.reviewNote}
        </p>
      )}
    </div>
  )
...
export function LearningAssessmentPanel({ scope, scopeId, courseId, title = 'Evaluaciones' }: { scope: Scope; scopeId: string; courseId: string; title?: string }) {
  ...
  const toggle = (id: string) => {
    if (activeId === id) { setActiveId(null); setDetail(null); setAnswers({}); setMessage(null); return }
    void open(id)
  }
  ...
  return <section className="rounded-2xl border border-zinc-700 bg-white/5 p-4 space-y-3">
    ...
    {detail && <div className="space-y-4 border-t border-zinc-700 pt-4"><div><h4 className="font-semibold text-ap-ivory">{detail.assessment.title}</h4>{detail.assessment.description && <p className="text-sm text-zinc-400 mt-1">{detail.assessment.description}</p>}<p className="text-xs text-zinc-500 mt-1">Intentos disponibles: {detail.remainingAttempts}</p></div>{detail.unavailableReason && <p className="rounded-lg bg-white/5 p-3 text-sm text-zinc-400">🔒 {detail.unavailableReason}</p>}{detail.waitingForReview && <p className="rounded-lg bg-ap-copper/10 p-3 text-sm text-ap-copper">⏳ Tu envío espera corrección administrativa.</p>}{detail.exhausted && <p className="rounded-lg bg-red-500/10 p-3 text-sm text-red-300">Agotaste los intentos. Comunícate con administración para solicitar una revalidación.</p>}
      ...
      {detail.attempts.length > 0 && <AttemptResult attempt={detail.attempts[0]} passingScore={detail.assessment.passingScore} />}</div>}
  </section>
}
