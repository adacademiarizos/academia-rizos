...
interface ModuleResourceManagerProps {
  moduleId: string
  onResourcesChange?: (resources: ModuleResource[]) => void
}
...
export default function ModuleResourceManager({
  moduleId,
  onResourcesChange,
  ...
  const fetchResources = async () => {
    try {
      setIsLoading(true)
      setError(null)
      const response = await fetch(`/api/admin/modules/${moduleId}/resources`)
  ...
  const handleDeleteResource = async (resourceId: string, title: string) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/modules/${moduleId}/resources/${resourceId}`,
        { method: 'DELETE' }
      )
    ...
    try {
      for (let i = 0; i < updatedResources.length; i++) {
        ...
        if (resource.order !== resources[i].order) {
          await fetch(`/api/admin/modules/${moduleId}/resources/${resource.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
