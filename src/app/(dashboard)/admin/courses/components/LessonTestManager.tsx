...
/** Matches the `.min(2)` the tests API enforces on each question's options. */
const MIN_OPTIONS = 2
...
export function LessonTestManager({ lessonId }: { lessonId: string }) {
  ...
  useEffect(() => { load().catch(() => toast.error('No fue posible cargar los tests.')) }, [load])
  ...
  // The API requires at least two options, so the last pair cannot be removed.
  const removeOption = (questionIndex: number, optionIndex: number) => {
    ...
    updateQuestion(questionIndex, {
      ...
      // Dropping the option that was marked correct would leave a dangling answer.
  ...
  const createTest = async () => {
    if (!title.trim() || questions.some((question) => !question.title.trim() || question.options.some((option) => !option.trim()) || !question.correctAnswer)) {
      toast.error('Completa el título y cada pregunta, opción y respuesta correcta.')
      return
    }
    setSaving(true)
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'No fue posible crear el test.')
  ...
  const remove = async (testId: string) => {
    ...
    try {
      ...
      const payload = await response.json()
      if (!response.ok) throw new Error(payload.error || 'No fue posible eliminar el test.')
      await load()
      toast.success('Test eliminado.')
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'No fue posible eliminar el test.')
    }
  }
