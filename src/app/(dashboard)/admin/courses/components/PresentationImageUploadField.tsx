...
export function PresentationImageUploadField({ label, value, onChange }: PresentationImageUploadFieldProps) {
  ...
  return <div className="space-y-3">
    ...
    {value && !isReplacing ? <div className="space-y-3">
      <button type="button" onClick={() => setIsPreviewOpen(true)} className="group block aspect-video w-full overflow-hidden rounded-2xl border border-white/15 bg-white/5 focus:outline-none focus:ring-2 focus:ring-ap-copper/70" aria-label="Ver imagen de presentación en tamaño grande">
        <img src={value} alt="Imagen de presentación" className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.03]" />
      </button>
    ...
    {showUploader ? <div className="space-y-2">
      <UploadFeedbackField
        key={value ?? 'empty'}
        label={value ? 'Reemplazar imagen o banner' : 'Cargar imagen o banner'}
    ...
    {isPreviewOpen && value ? createPortal(<div id={dialogId} role="dialog" aria-modal="true" aria-label="Vista previa de la imagen de presentación" className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm" onClick={() => setIsPreviewOpen(false)}>
      <div className="relative flex max-h-[90vh] max-w-5xl items-center justify-center rounded-2xl border border-white/15 bg-[#181716] p-2 shadow-2xl" onClick={(event) => event.stopPropagation()}>
        <img src={value} alt="Imagen de presentación ampliada" className="max-h-[85vh] max-w-full rounded-xl object-contain" />
