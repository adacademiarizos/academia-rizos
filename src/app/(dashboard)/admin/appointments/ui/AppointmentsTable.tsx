...
type StaffEntry = { id: string; name: string | null; email: string };
...
const PAY_STATUS: Record<string, { label: string; style: string }> = {
  ...
  CANCELED:          { label: "Cancelado",    style: "bg-red-500/20 text-red-300" },
};
...
// ─── componente: staff select custom ─────────────────────────────────────────
function StaffSelect({
  ...
  const selectedEntry = staffList.find((s) => s.id === value);
  ...
  return (
    <div ref={wrapRef} className="relative">
      ...
      >
        <div className="flex items-center gap-2">
          <span className="h-2 w-2 rounded-full bg-ap-copper/60 shrink-0" />
      ...
      {open && (
        <div className="absolute top-full left-0 right-0 mt-1 z-50 rounded-2xl border border-white/10 bg-[#1d1b19] shadow-2xl overflow-hidden">
          <div className="max-h-52 overflow-y-auto nav-scroll py-1">
            {[{ id: "ALL", name: "Todos los profesionales", email: "" }, ...staffList].map((s) => {
              const isActive = (s.id === "ALL" ? "ALL" : s.id) === value;
              const displayName = s.id === "ALL" ? s.name : (s.name ?? s.email);
              return (
                <button
                  key={s.id}
                  onClick={() => { onChange(s.id === "ALL" ? "ALL" : s.id); setOpen(false); }}
                  className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-left transition
...
export default function AppointmentsTable({
  ...
  }, [rows, filterDate, filterName, filterStatus, filterStaff]);
      ...
      <div className="rounded-3xl border border-white/10 bg-white/5 p-4 backdrop-blur-xl">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
          ...
          <div className="flex flex-col gap-1 min-w-[200px]">
            <label className="text-xs text-white/50">Profesional</label>
            <StaffSelect staffList={staffList} value={filterStaff} onChange={setFilterStaff} />
          </div>

          {/* Reset */}
          <button
            onClick={() => { setFilterName(""); setFilterDate(today); setFilterStatus("ALL"); setFilterStaff("ALL"); }}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {filtered.map((r) => {
            ...
            const durationMin = calcDuration(r.startAt, r.endAt);
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2 mb-4">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-[10px] text-white/30 mt-0.5">{durationMin} min</p>
                  </div>
                </div>
                ...
                <div className="space-y-2.5 flex-1">
                  ...
                  <div className="flex items-start gap-2">
                    <span className="mt-0.5 text-white/30 shrink-0 text-base">👤</span>
                    <div>
                  ...
                  {/* servicio + variante */}
                  <div className="flex items-start gap-2">
                    <span className="mt-0.5 text-white/30 shrink-0 text-base">✂️</span>
                    <div>
                      <p className="text-sm text-white/80 leading-tight">{r.service.name}</p>
                      {r.variant && (
